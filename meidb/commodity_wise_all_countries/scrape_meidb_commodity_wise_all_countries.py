...
from lib import create_session, bootstrap_session, fetch_commodity_countries_data, parse_commodity_countries_response, save_data
...
def main():
    parser = argparse.ArgumentParser(description="Scrape MEIDB commodity-wise all countries data")
    ...
    print(f"[*] Scraping MEIDB COMMODITY-WISE ALL COUNTRIES {args.trade_type.upper()} data...")
    ...
    state = bootstrap_session(session, BASE_URL, f"/meidb/commodity_wise_all_countries_{args.trade_type}")
    print("[+] Session bootstrapped")
    
    html = fetch_commodity_countries_data(
    ...
    data = parse_commodity_countries_response(html, args.hscode, args.month, args.year, args.trade_type, args.value_type, args.year_type)
    if not data:
