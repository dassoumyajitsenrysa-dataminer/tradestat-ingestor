"""
Parser for Region-wise all Commodities TradeStat HTML responses.
Extracts trade data with commodity breakdown from HTML tables.
"""
...
def parse_region_commodities_response(
    ...
    """
    Parse the HTML response and extract region-wise commodities data.
    ...
    Args:
        ...
        hs_code: Specific HS code to filter, or "all"
        ...
        for row in rows:
            ...
            if len(cells) >= 3:
                ...
                if record:
                    ...
                    if "total" in commodity or "india's total" in commodity:
                        total_record = record
                    else:
    ...
    result = {
        "metadata": {
            "source": {
                ...
                "url": f"https://tradestat.commerce.gov.in/eidb/region_wise_all_commodities_{trade_type}",
                "database": "EIDB (Export Import Data Bank)"
            },
            "extraction": {
                "timestamp_utc": datetime.now(timezone.utc).isoformat(),
                "timestamp_ist": datetime.now(IST).isoformat(),
                "tool": "region_wise_all_commodities_scraper",
                "version": "1.0.0"
            },
            "query_parameters": {
            ...
            "data_info": {
                ...
                "notes": [
                    ...
                    "Quantity is available only at 8 digit level",
                    "* ITC HS Code of the Commodity is either dropped or re-allocated"
                ],
                "warning": None if exact_match_found else f"HS code '{hs_code}' not found in database for region {region_name}"
            }
        },
        "data": {
            "commodities": records,
...
def parse_row(cells: list, value_type: str, hs_level: str) -> Optional[dict]:
    ...
    Expected columns: S.No | HSCode | Commodity | PrevYear | CurrYear | %Growth
    ...
    try:
        ...
        if not first_cell_text:
            return None
        if first_cell_text.lower() in ['s.no', 's.no.', 'sno', 'sl.no', '#', 'report dated']:
            return None
        ...
        # Table structure: S.No | HSCode | Commodity | PrevYear | CurrYear | %Growth
        if len(cells) >= 6:
            ...
            record["percentage_growth"] = parse_numeric(cells[4].get_text(strip=True))
        elif len(cells) >= 3:
            ...
            if "total" in combined_text.lower():
                ...
                for cell in cells:
                    ...
                    if val is not None and val > 100:  # Likely a total value
